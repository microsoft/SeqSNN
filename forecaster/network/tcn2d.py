...
from forecaster.module import Chomp2d, PositionEmbedding
from forecaster.network.tcn import TemporalBlock
...
@NETWORKS.register_module("TCN2D_Decoder")
class TemporalConvNet2D_Decoder(TSRNN):
    def __init__(
        self,
        # num_channels: List[int],
        num_levels: int,
        ...
        num_channels = [channel] * num_levels
        num_channels.append(1)
        # print('====TCN2d network=====: ', len(num_channels), out_size)
        for i in range(num_levels + 1):
            dilation_size = dilation**i
            in_channels = out_size if i == 0 else num_channels[i - 1]
            out_channels = num_channels[i]
            # print(out_channels)
            layers += [
                TemporalBlock2D(
...
class TemporalConvNetLayer(TSRNN):
    def __init__(
        self,
        # num_channels: List[int],
        num_levels: int,
