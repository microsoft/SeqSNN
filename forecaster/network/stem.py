...
class StockBlockLayer(nn.Module):
    def __init__(self, time_step, unit, multi_layer, stack_cnt=0):
        ...
        for i in range(3):
            if i == 0:
                self.GLUs.append(GLU((self.time_step // 2 + 1) * 4, self.time_step * self.output_channel))
                self.GLUs.append(GLU((self.time_step // 2 + 1) * 4, self.time_step * self.output_channel))
            elif i == 1:
    ...
    def spe_seq_cell(self, input):
        ...
        iffted = torch.fft.irfft(torch.complex(real, img), self.time_step * self.multi)
        return iffted
...
class StemGNN(nn.Module):
    def __init__(
        ...
        """Implementation of StemGNN in https://arxiv.org/abs/2103.07719.
        From: https://github.com/microsoft/StemGNN


        Args:
            multi_layer: Number of StemGNN layers.
            stack_cnt: Number of StemGNN stacks, 1 or 2.
            dropout: Dropout rate
            leaky_rate: The leaky rate of leaky relu activation.
        """
        super(StemGNN, self).__init__()
        self.unit = input_size
        self.stack_cnt = stack_cnt
        self.alpha = leaky_rate
        self.time_step = max_length
    ...
    def forward(self, x):
        ...
        forecast = result[0] + result[1]  # B, U, L
        forecast = self.fc(forecast)
        return forecast.transpose(1, 2), forecast.mean(dim=-1)
